...
CREATE OR REPLACE TRIGGER trg_set_history_insert AFTER INSERT ON pyz_time_slot FOR EACH ROW
    ...
    SELECT SYSDATE(), NEW.merchant_reference, null, NEW.time_slot, NEW.day, NEW.date, NEW.capacity, 'INSERT', 1
...
CREATE OR REPLACE TRIGGER trg_set_history_update AFTER UPDATE ON pyz_time_slot FOR EACH ROW
    ...
    SELECT SYSDATE(), NEW.merchant_reference, null, NEW.time_slot, NEW.day, NEW.date, OLD.capacity, NEW.capacity, 'UPDATE', 1
    WHERE NEW.created_at is NULL;
delimiter //
CREATE OR REPLACE TRIGGER trg_set_history_delete AFTER DELETE ON pyz_time_slot FOR EACH ROW
    ...
    SELECT SYSDATE(), OLD.merchant_reference, null, OLD.time_slot, OLD.day, OLD.date, OLD.capacity, 'DELETE', 1
    WHERE OLD.created_at is NULL;
