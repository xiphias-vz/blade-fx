 ...
 */
class ConfirmDeliveryCommandPlugin extends AbstractPlugin implements CommandByOrderInterface
{
    /**
     * @api
     *
    ...
    {
        $currentDateTime = $this->getFactory()->getDateTimeWithZoneService()->getDateTimeInStoreTimeZone('now');

        $orderUpdateRequestTransfer = (new OrderUpdateRequestTransfer())
            ->setStoreStatus(OmsConfig::STORE_STATE_DELIVERED)
            ->setDeliveredAt($currentDateTime);
