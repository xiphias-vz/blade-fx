...
class SalesOverviewRepository extends AbstractRepository implements SalesOverviewRepositoryInterface
{
    ...
    public function getDataByChoiceFilter(int $choice, string $deliveryDate, string $merchantReference): array
    {
        ...
        foreach ($result as $timeSlotKey => $timeSlotValue) {
            foreach ($timeSlotValue as $pickZone) {
                foreach ($pickZone as $stateKey => $stateValue) {
                    if (!array_key_exists("Total", $result[$timeSlotKey])) {
                        $result[$timeSlotKey]["Total"][$stateKey] = 0;
                    } elseif (!array_key_exists($stateKey, $result[$timeSlotKey]["Total"])) {
                        $result[$timeSlotKey]["Total"][$stateKey] = 0;
                    }
                    $result[$timeSlotKey]["Total"][$stateKey] = $result[$timeSlotKey]["Total"][$stateKey] + $stateValue;
                }
            }
        }
